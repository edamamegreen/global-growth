...
(defn quotify
  [quote author]
  (str quote "\n\n-- " author)) ;; any functions that are called by main need to be placed above main.

(defn get-population-density []
  (json/parse-string (get (client/get "http://api.worldbank.org/countries/all/indicators/EN.POP.DNST?format=json&date=2010") :body) true))
...
(defn get-country-and-value
  [response]
  (for [item (:results response)] ;; I'm not sure I understand the for function, i believe that it is creating item here as a variable.
...
(defn -main
  ...
  ;(println (quotify "A man who carries a cat by the tail learns something he can learn in no other way." "Mark Twain"))
  (println (take 10 (reverse (sort-by second (get-indicator-values "EN.POP.DNST" 2010))))))
